        ...
        if len(parameter):
            ...
            if file_utils.is_digit(board_height):
                ...
                if not attr_values.count():
                    ...
                    attr_val = UnFixedAttributeValue(value=float(board_height), attribute=attribute, created_by=user,
                                                     updated_by=user)
                    attr_val.save()
            ...
            if file_utils.is_digit(width):
                ...
                if not attr_values.count():
                    ...
                    attr_val = UnFixedAttributeValue(value=float(width), attribute=attribute, created_by=user,
                                                     updated_by=user)
                    attr_val.save()
                    attr_val.products.add(product)
                    attr_val.save()
            ...
            if file_utils.is_digit(thickness):
                ...
                if not attr_values.count():
                    ...
                    attr_val = UnFixedAttributeValue(value=float(thickness), attribute=attribute, created_by=user,
                                                     updated_by=user)
                    attr_val.save()
                    attr_val.products.add(product)
                    attr_val.save()
